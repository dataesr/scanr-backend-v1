            ...
            controller: ["$scope", "$http", "$location", "$filter", "$sce", "SchedulingApi",function ($scope, $http, $location, $filter, $sce, SchedulingApi) {
                ...
                function buildQueueNode(e, select, desc, name) {
                    ...
                    $scope.handlers[name] = function (maxCount) {
                        var q = $scope.status.data.queues[name]; // [SWORD] .data else $scope.status.queues is undefined l103 284 24 svg-monitoring.js
                        var q_out = $scope.status.data.queues[name + "_OUT"]; // [SWORD] .data
                        if (titleElt) {
                        ...
                        var errCount = $scope.status.data.errors[name]; // [SWORD] .data
                        if (q_out) {
                            errCount += $scope.status.data.errors[name + "_OUT"]; // [SWORD] .data
                        }
                ...
                function buildExchangeNode(e, select, name) {
                    ...
                    $scope.handlers[name] = function () {
                        select.text($scope.status.data.exchanges[name] + "/m"); // [SWORD] .data
                    };
                ...
                function buildErrorCountNode(e, select, name) {
                    ...
                    $scope.handlers[name] = function () {
                        countNode.text($scope.status.data.totalErrors); // [SWORD] .data
                    };
                ...
                function buildScheduleNode(e, select, desc, name) {
                    ...
                    var circleStatus = buildSVGElement(parent,'circle')
                        ...
                        .attr("r",6);
                    $scope.handlers[name] = function () {
                        var scheduledMessage = $scope.status.data.scheduledMessages[name]; // [SWORD] .data
                        if (scheduledMessage) {
                    ...
                    maskForClick.bind("click", function (e) {
                        ...
                        if ($scope.status.data.scheduledMessages[name]) { // [SWORD] .data
                            var st = $scope.status.data.scheduledMessages[name].scheduledMessage; // [SWORD] .data
                            ...
                            $scope.tooltip.text = $sce.trustAsHtml("<div class='title'>" + desc.label + "</div>"
                                ...
                                + "<div>When: " + $scope.status.data.scheduledMessages[name].triggerHuman + "</div>" // [SWORD] .data
                ...
                $scope.updateSVG = function () {
                    ...
                    var totalMessages = 0;
                    var totalErrors = 0;
                    for (var q in $scope.status.data.queues) { // [SWORD] .data
                        if (!$scope.queues[q]) {
                            continue;
                        }
                        var count = $scope.status.data.queues[q].count; // [SWORD] .data
                        var errors = $scope.status.data.errors[q]; // [SWORD] .data
                        totalMessages += count;
                        totalErrors += errors;
