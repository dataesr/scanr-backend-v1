...
/**
 * Impl of BufferMessageContainer that enables buffering
 * Copied from the original spring impl... with shame
 */
public class BufferMessageContainer extends AbstractMessageListenerContainer
    ...
    private final ContainerDelegate delegate = new ContainerDelegate() {
        ...
        public void invokeListener(Channel channel, Message message) throws Exception {
            BufferMessageContainer.super.invokeListener(channel, message);
        }
    };
     ...
     */
    public BufferMessageContainer(ConnectionFactory connectionFactory) {
    ...
    public void setConcurrentConsumers(final int concurrentConsumers) {
        ...
        synchronized(consumersMonitor) {
            ...
            if (isActive() && this.consumers != null) {
                if (delta > 0) {
                    Iterator<Map.Entry<BlockingQueueConsumer, Boolean>> entryIterator = consumers.entrySet()
                            .iterator();
     ...
     */
    @Override
    public void addQueues(org.springframework.amqp.core.Queue... queue) {
     ...
     */
    @Override
    public boolean removeQueues(org.springframework.amqp.core.Queue... queue) {
    ...
    protected void validateConfiguration() {
        ...
        if (this.getConnectionFactory() instanceof CachingConnectionFactory) {
            ...
            if (cf.getCacheMode() == CachingConnectionFactory.CacheMode.CHANNEL && cf.getChannelCacheSize() < this.concurrentConsumers) {
    ...
    private synchronized void redeclareElementsIfNecessary() {
        try {
            ...
            if (applicationContext != null) {
                ...
                Map<String, org.springframework.amqp.core.Queue> queueBeans = applicationContext.getBeansOfType(org.springframework.amqp.core.Queue.class);
                for (Map.Entry<String, org.springframework.amqp.core.Queue> entry : queueBeans.entrySet()) {
                    org.springframework.amqp.core.Queue queue = entry.getValue();
    ...
    private boolean doReceiveAndExecute(BlockingQueueConsumer consumer) throws Throwable {
        ...
        if (buffer.size() != 0) {
            try {
                executeListener(channel, new MultiMessage(buffer));
            } catch (ImmediateAcknowledgeAmqpException ignored) {
            } catch (Throwable ex) {
    ...
    public String toString() {
        return "BufferMessageContainer [concurrentConsumers=" + this.concurrentConsumers
    ...
    private class AsyncMessageProcessingConsumer implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                try {
                    if (BufferMessageContainer.this.autoDeclare) {
                        BufferMessageContainer.this.redeclareElementsIfNecessary();
                    }
                    this.consumer.start();
                    this.start.countDown();
                }
                catch (QueuesNotAvailableException e) {
                    if (BufferMessageContainer.this.missingQueuesFatal) {
                        throw e;
                    }
                    else {
                        this.start.countDown();
                ...
                if (BufferMessageContainer.this.transactionManager != null) {
					/*
                ...
                while (isActive(this.consumer) || this.consumer.hasDelivery()) {
                    try {
                        ...
                        if (BufferMessageContainer.this.maxConcurrentConsumers != null) {
                            if (receivedOk) {
                                if (isActive(this.consumer)) {
                                    consecutiveIdles = 0;
                                    if (consecutiveMessages++ > BufferMessageContainer.this.consecutiveActiveTrigger) {
                                ...
                                consecutiveMessages = 0;
                                if (consecutiveIdles++ > BufferMessageContainer.this.consecutiveIdleTrigger) {
            ...
            if (!isActive(consumer) || aborted) {
                ...
                try {
                    this.consumer.stop();
                    BufferMessageContainer.this.cancellationLock.release(this.consumer);
                    synchronized (consumersMonitor) {
                        if (BufferMessageContainer.this.consumers != null) {
                            BufferMessageContainer.this.consumers.remove(this.consumer);
                        }
                    }
                }
        ...
        private void publishEvent(String reason, boolean fatal, Throwable t) {
            if (applicationEventPublisher != null) {
                applicationEventPublisher.publishEvent(new ListenerContainerConsumerFailedEvent(
                        BufferMessageContainer.this, reason, t, fatal));
            }
        }
